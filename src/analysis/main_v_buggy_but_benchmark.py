...
def main():
    ...
    print(f"Constituents Data Path: {constituents_data_path}")
    ...
    stats = summarize_performance(excess_returns, rf_monthly, spi_returns_monthly, 12)

    plot_cumulative_returns(excess_returns, benchmark=spi_XsReturns_monthly, filename=visualization_path / "cumulative_returns")
    ...
    # Over Holding Period
    # Loop over 1-12
    rc_holding_period = pd.DataFrame(index=range(1, 13), columns=['Sharpe Ratio'])
    for i in range(1,13):
        ...
        rc_holding_period.loc[i,'Sharpe Ratio'] = stats_temp['Sharpe_Ratio_Arithmetic']['xs_Return']
    ...
    # Loop over 1-12
    rc_lookback_period = pd.DataFrame(index=range(1, 13), columns=['Sharpe Ratio'])
    for i in range(1,13):
        excess_returnsTemp , _ , _, _ = momentum_strategy(
            price_data_daily=price_data_daily,
            lookback_period=i,
        ...
        excess_returnsTemp.columns = ['Strategy_Returns']
        stats_temp = summarize_performance(excess_returnsTemp, rf_monthly, spi_XsReturns_monthly, 12)
        rc_lookback_period.loc[i,'Sharpe Ratio'] = stats_temp['Sharpe_Ratio_Arithmetic']['xs_Return']
    ...
    # Over Number Assets
    # Loop over 5-50
    rc_number_assets = pd.DataFrame(index=range(5, 51), columns=['Sharpe Ratio'])
    for i in range(5,51):
        excess_returnsTemp , _ , _, _ = momentum_strategy(
            price_data_daily=price_data_daily,
            lookback_period=lookback_period,
        ...
        excess_returnsTemp.columns = ['Strategy_Returns']
        stats_temp = summarize_performance(excess_returnsTemp, rf_monthly, spi_XsReturns_monthly, 12)
        rc_number_assets.loc[i,'Sharpe Ratio'] = stats_temp['Sharpe_Ratio_Arithmetic']['xs_Return']
    ...
    # plot here
    plot_cumulative_returns(rc_trxCost_XsRet, benchmark=spi_returns_monthly, title='Robustness Check Transaction Costs', x_label='Date', y_label='Cumulative Returns', figsize=(12,6), grid=True, savefig=True, filename=visualization_path / 'rc_trxCost.png')
